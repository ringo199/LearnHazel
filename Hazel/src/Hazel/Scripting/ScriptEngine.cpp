...
namespace Hazel
{
	...
	void PrintAssemblyTypes(MonoAssembly* assembly)
	{
		...
		for (int32_t i = 0; i < numTypes; i++)
		{
			...
			const char* method = mono_metadata_string_heap(image, cols[MONO_TYPEDEF_METHOD_LIST]);

			HZ_CORE_TRACE("{}.{} - {}", nameSpace, name, method);
		}
	}
	...
	void ScriptEngine::InitMono()
	{
		...
		MonoMethod* PrintCustemMessageFunc = mono_class_get_method_from_name(monoClass, "PrintCustumMessage", 1);
		
		void* stringParam = monoString;
		mono_runtime_invoke(PrintCustemMessageFunc, instance, &stringParam, nullptr);
